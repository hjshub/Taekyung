...
(function($){
	...
	function $Func(){
		...
		*/
		...
		document.title = tkObj.$title.text() + ' | Taekyung'; 
		...
		SnsSlider = function(){ //sns slider
			tkObj.snsSlider = $('#section05 .sns-list').slick({
				arrows:false,
				dots: true,
				infinite: true,
				speed: 600,
		...
		hoverFunc = function(){
			tkObj.brandInfo = $('#section04 .brand-info'),
	...
	window.onload = function(){
		...
		$(this).on("scroll", function() {
			if(!tkObj.isMobile){
				...
				if(tkObj.$scrollPos >= 1 && tkObj.$scrollPos < 3){ // section 2,3
